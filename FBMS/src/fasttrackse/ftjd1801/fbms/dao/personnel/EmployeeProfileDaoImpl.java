package fasttrackse.ftjd1801.fbms.dao.personnel;

import java.util.List;

import org.hibernate.Criteria;
import org.hibernate.criterion.Criterion;
import org.hibernate.criterion.Restrictions;
import org.springframework.stereotype.Repository;

import fasttrackse.ftjd1801.fbms.entity.personnel.EmployeeProfile;

@Repository("EmployeeProfileDao")
public class EmployeeProfileDaoImpl extends AbstractDao<Integer, EmployeeProfile> implements EmployeeProfileDao {
	public EmployeeProfile findById(int id) {
		return getByKey(id);
	}

	@Override
	public void saveEmployeeProfile(EmployeeProfile EmployeeProfile) {
		persist(EmployeeProfile);
	}

	@Override
	public void deleteEmployeeProfileById(int id) {
		delete(getByKey(id));
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<EmployeeProfile> findAllEmployeeProfiles() {
		Criteria criteria = createEntityCriteria();
		return (List<EmployeeProfile>) criteria.list();
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<EmployeeProfile> findAllEmployeeProfiles(String search) {
		Criteria criteria = createEntityCriteria();
		Criterion cond = Restrictions.ilike("name", "%" + search + "%");
		criteria.add(cond);
		return (List<EmployeeProfile>) criteria.list();
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<EmployeeProfile> getEmployeeProfiles(int fromIndex, int toIndex) {
		Criteria criteria = createEntityCriteria();
		return (List<EmployeeProfile>) criteria.list().subList(fromIndex, toIndex);
	}

	@SuppressWarnings("unchecked")
	public List<EmployeeProfile> getEmployeeProfiles(int fromIndex, int toIndex, String search) {
		Criteria criteria = createEntityCriteria();
		Criterion cond = Restrictions.ilike("name", "%" + search + "%");
		criteria.add(cond);
		return (List<EmployeeProfile>) criteria.list().subList(fromIndex, toIndex);
	}

}
